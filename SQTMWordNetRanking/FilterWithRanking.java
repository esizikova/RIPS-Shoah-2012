...
class FilterWithRanking {

    public static List<String> filterWithRanking(List<String> words, String[][] filterWords, String[] stopWords) throws IOException {
    	...
    	double allScore = 0;
    	for (int h = 0; h < scoreList.length; h++){
    		allScore += scoreList[h];
    	}
    	//System.out.println("All Score = " + allScore);
    	double thresh = (allScore / words.size()) * .5;
    	//System.out.println("Thresh = " + thresh);
    	for (int j = 0; j < words.size(); j++){
    		if (scoreList[j] >= thresh){
    			finalList.add(wordsList[j]);
    		}
    	}
